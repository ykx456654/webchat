	...
	function detectType(media, options, src) {
		...
		if (zyMedia.features.supportsCanPlayType) {
			for (i = 0; i < mediaFiles.length; i++) {
				// Normal detect
				if (mediaFiles[i].type == "application/x-mpegURL"|| mediaFiles[i].type == "video/m3u8" || media.canPlayType(mediaFiles[i].type).replace(/no/, '') !== '' ||
...
// String or node
export default function zy (selector, options) {
	if (typeof selector === 'string') {
		[].forEach.call(document.querySelectorAll(selector), function(el) {
			return new zyMedia.MediaPlayer(el, options)
		});
	} else {
